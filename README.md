...
Run the notebooks in order: `01_Load_clean_data_carla.ipynb` → `Step_1_data_exploration_Kseniia.ipynb` → `ML_Student_Dropout_nadiya.ipynb` (KNN) → `02_Random_forest_carla.ipynb` (Random Forest) → `Step_2_model_selection_Kseniia.ipynb` (Gradient Boosting).
